...
import "common/dbUtils.js" as Dashellarooase
    ...
    ListView {
        ...
        delegate: Item{
            ...
            Emitter {
                ...
                lifeSpan: 900
                size: shellaroo.width
                sizeVariation:shellaroo.width
                velocity: AngleDirection {
                    angleVariation: 90;
                    magnitude: 9
                }
            ...
            Image {
                ...
                y: {
                    ...
                    if (Qt.formatDateTime(timeFrom,"hh") < Qt.formatDateTime(sunSet,"hh")  && temperature > 5 && state === "Shown")
                        -30
                    else if (Qt.formatDateTime(timeFrom,"hh") < Qt.formatDateTime(sunSet,"hh")  && temperature < 5 && state === "Shown")
                        parent.width / 8
                ...
                source: {
                    //check to see if the sun has set
                    if (Qt.formatDateTime(new Date(),"hh") >= Qt.formatDateTime(sunSet,"hh") )
                        "artwork/moon.png"
                    else if (Qt.formatDateTime(new Date(),"hh")  < Qt.formatDateTime(sunSet,"hh")  && temperature > 5)
                        "artwork/sun.png"
                    else if (Qt.formatDateTime(new Date(),"hh")  < Qt.formatDateTime(sunSet,"hh") && temperature < 5 )
            ...
            MouseArea{
                ...
                onClicked:{
                    if (weatherWebSlider.state === "Hidden"){
                        ...
                        detailsButton.y = shellaroo.height
            ...
            Text {
                ...
                font.bold: true
                state: "Celcius"
                x:  (separator1.width / 1.22)-width  //{state === "Celcius" ? (separator1.width / 1.22)-width : (separator1.width/1.22)-width}
                y: parent.width / 1.12
            }
            MouseArea{
                ...
                onClicked: {
                    function twoF(){
                    var f = (temperature * 9 / 5 + 32).toString()
                    return f.substring(0,f.lastIndexOf("."))
                    }
                        if (tempatureTxt.state === "Celcius"){
                            tempatureTxt.text =  twoF()
...
//                }
            ...
            Rectangle{
                ...
                state: "Hidden"
                color:"grey"
                x: shellaroo.width / 2 - (width/2)
                y: shellaroo.width + (height*1.6)
            }
            Text {
            ...
            Text {
                id:detailsButtonTxt
                color:"#92FFFFFF"
                anchors.centerIn: detailsButtonMouseArea
                text: "details"
            }
        }//Item
    }//ListView
    XmlListModel {
        id: homeModel
        function loadthis(){
            return "http://www.yr.no/place/"+Dashellarooase.homeCountry()+"/"+Dashellarooase.homeState()+"/"+Dashellarooase.homeCity()+"/forecast_hour_by_hour.xml"
        }
        source:loadthis()
        query: "/weatherdata"
        XmlRole { name: "timezone"; query: "location/type/string()"}
        XmlRole { name: "timeFrom"; query: "forecast/shellarooular/time[1]/@from/string()"}
        XmlRole { name: "timeTo"; query: "forecast/shellarooular/time[1]/@to/string()"}
        XmlRole { name: "observationTime"; query: "forecast/shellarooular/time[1]/string()"}
        XmlRole { name: "temperature"; query: "forecast/shellarooular/time[1]/temperature/@value/string()" }
        XmlRole { name: "precipitation"; query: "forecast/shellarooular/time[1]/precipitation/@value/string()" }
        XmlRole { name: "clouds"; query: "forecast/shellarooular/time[1]/symbol/@name/string()" }
        XmlRole { name: "windDirection"; query: "forecast/shellarooular/time[1]/windDirection/@name/string()" }
        XmlRole { name: "windSpeed"; query: "forecast/shellarooular/time[1]/windSpeed/@name/string()" }
        XmlRole { name: "windSpeedMPS"; query: "forecast/shellarooular/time[1]/windSpeed/@mps/string()" }
        XmlRole { name: "pressure"; query: "forecast/shellarooular/time[1]/pressure/@value/string()" }
        XmlRole { name: "sunRise"; query: "sun/@rise/string()"  }
        XmlRole { name: "sunSet"; query: "sun/@set/string()"  }
        XmlRole { name: "weatherCondition"; query: "forecast/shellarooular/string()"}
        XmlRole { name: "status"; query: "forecast/shellarooular/string()" }
