...
Item {
    ...
    XmlListModel {
        ...
        XmlRole { name: "pid"; query: "ChanId/string()" }
        XmlRole { name: "start"; query: "Programs/Program/StartTime/string()" }
        XmlRole { name: "end"; query: "Programs/Program/EndTime/string()" }
        XmlRole { name: "title"; query: "Programs/Program/Title/string()" }

        onStatusChanged: {
            if (status == XmlListModel.Ready) {
                if (schedule.count == 0) return;

                var startOfFirst = Util.updateTime(schedule.get(0).start)
                var endOfLast = Util.updateTime(schedule.get(schedule.count - 1).end)
                ...
                for (var i = 0; i < schedule.count; i++) {
                    var start = Util.updateTime(schedule.get(i).start)
                    var end = Util.updateTime(schedule.get(i).end)
                    broadcasts.append({ 'start': start, 'end': end,
                                        'title': schedule.get(i).title, isOffAir: false })

                    if (i < schedule.count - 1) {
                        var startOfNext = Util.updateTime(schedule.get(i + 1).start)
    ...
    Item {
        ...
        Repeater {
            ...
            delegate: Broadcast {
                ...
                width: (model.end - model.start) * channel.hourSize / Util.MS_PER_HOUR
                x: (model.start - channel.startOfTimeline) * channel.hourSize / Util.MS_PER_HOUR
...
//    XmlListModel {
//        id: schedule
//        source: dash2dConfiguration.mythipBackend + "/Guide/GetProgramGuide?StartTime="+Util.now.toISOString()+"&EndTime="+Util.time2.toISOString()
//        query: "//ProgramGuide//*"
