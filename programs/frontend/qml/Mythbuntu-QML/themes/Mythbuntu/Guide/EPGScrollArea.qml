...
Flickable {
    ...
    contentWidth: hourSize * (area.endOfTimeline - area.startOfTimeline) / Util.MS_PER_HOUR
    contentHeight: channelModel.count * channelHeight
    clip: true
    ...
    Timer {
        ...
        onTriggered: {
            ...
            if (currentTime && startOfTimeline && selectedChannelItem) {
                ...
                var newContentX = hourSize * (currentTime - startOfTimeline) / Util.MS_PER_HOUR - area.width / 2
                ...
                area.referenceTime = new Date(area.startOfTimeline.getTime() + (pixelReference / hourSize * Util.MS_PER_HOUR))
            }
        }
    }
    ...
    Keys.onRightPressed: {
        ...
        var broadcastX = hourSize * (broadcast.start - area.startOfTimeline) / Util.MS_PER_HOUR
        ...
        var nextX = (next) ? hourSize * (next.start - area.startOfTimeline) / Util.MS_PER_HOUR : 0
        ...
        if (broadcastX - area.contentX > area.width - area.horizontalScrollMargin ||
            ...
            targetX = Util.clamp(broadcastX - area.horizontalScrollMargin,
    ...
    Keys.onLeftPressed: {
        ...
        if (broadcastX - area.contentX < area.horizontalScrollMargin) {
            var broadcastWidth = hourSize * (broadcast.end - broadcast.start) / Util.MS_PER_HOUR
        ...
        targetX = Util.clamp(targetX, 0, area.contentWidth - area.width)
        area.contentX = targetX
        calculateReferenceTime(broadcast, targetX)
    }

    function calculateReferenceTime(broadcast, newContentX) {
        var x = hourSize * (broadcast.start - area.startOfTimeline) / Util.MS_PER_HOUR
        var size = hourSize * (broadcast.end - broadcast.start) / Util.MS_PER_HOUR
